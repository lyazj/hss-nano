...
%s)'''
    ...
    if isdata:
        if '2016' in prepid:
            year = '2016APV' if 'HIPM' in prepid else '2016'
        elif '2017' in prepid:
            year = '2017'
        elif '2018' in prepid:
            year = '2018'
    ...
    # Only create local log directory, not remote storage path
    if os.system("mkdir -p '%s'" % logdir):
        raise RuntimeError('error making log directory')
    queue = ''
    ...
    jobfile = dataset_name + '_' + prepid + '.jdl'
